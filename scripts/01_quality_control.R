...
# Before filtering: check the data
...
p2 <- FeatureScatter(pbmc,
                     feature1 = "nCount_RNA", 
                     feature2 = "percent.mt") +
  geom_hline(yintercept = c(5, 10, 15), # Draw thresholds at various levels
             linetype = c("dashed", "dashed", "dashed"), 
             color = c("purple", "darkgreen", "black")) +
  ggtitle("RNA Count vs Mitochondrial Percentage")

p3 <- FeatureScatter(pbmc,
                     feature1 = "nCount_RNA",
                     feature2 = "nFeature_RNA") +
  geom_vline(xintercept = c(500, 5000), 
             linetype = "dashed", 
             color = "red") +
  geom_hline(yintercept = c(250, 2500), 
             linetype = "dashed", 
             color = "red") +
  ggtitle("RNA Count vs Number of Genes")

# Save QC plots
dir.create("results/figures", recursive = TRUE, showWarnings = FALSE)
ggsave("results/figures/01_qc_violin_plots.png",
       plot = p1,
       width = 12,
       height = 6)
ggsave("results/figures/02_qc_scatter_mt.png",
       plot = p2,
       width = 8,
       height = 6)
ggsave("results/figures/03_qc_scatter_features.png",
       plot = p3,
       width = 8,
       height = 6)
...
cat("\nQC complete! Check results/figures/ for QC plots.\n")
